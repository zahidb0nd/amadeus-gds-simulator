...
const edgeCases = [
  // 1. AN edge case
  { type: 'INVALID_AN_FORMAT', action: async (sid, o, d) => {
      return await executeCommand(`AN20AUG${o}`, { sessionId: sid }); // Missing destination
  }, expected: 'INVALID FORMAT' },
  
  // 2. SS edge case
  { type: 'INVALID_LINE_SS', action: async (sid, o, d) => {
  ...
  // 6. FXP edge case
  { type: 'PRICE_WITHOUT_ITINERARY', action: async (sid, o, d) => {
      return await executeCommand(`FXP`, { sessionId: sid });
  }, expected: 'NO ITINERARY TO PRICE' },

  // 7. TTP edge case
  { type: 'TTP_WITHOUT_FXP', action: async (sid, o, d) => {
      await executeCommand(`AN20AUG${o}${d}`, { sessionId: sid });
      await executeCommand(`SS1Y1`, { sessionId: sid });
      await executeCommand(`NM1TEST/PASSENGER`, { sessionId: sid });
  ...
  // 8. XI edge case
  { type: 'CANCEL_NONEXISTENT_SEGMENT', action: async (sid, o, d) => {
      await executeCommand(`AN20AUG${o}${d}`, { sessionId: sid });
      await executeCommand(`SS1Y1`, { sessionId: sid });
      await executeCommand(`NM1TEST/PASSENGER`, { sessionId: sid });
      await executeCommand(`AP+9715550000`, { sessionId: sid });
  ...
  // 9. REFUND edge case
  { type: 'REFUND_WITHOUT_CANCEL', action: async (sid, o, d) => {
      await executeCommand(`AN20AUG${o}${d}`, { sessionId: sid });
      await executeCommand(`SS1Y1`, { sessionId: sid });
      await executeCommand(`NM1TEST/PASSENGER`, { sessionId: sid });
      await executeCommand(`AP+9715550000`, { sessionId: sid });
  ...
  { type: 'VOID_WITHOUT_TICKET', action: async (sid, o, d) => {
      await executeCommand(`AN20AUG${o}${d}`, { sessionId: sid });
      await executeCommand(`SS1Y1`, { sessionId: sid });
      await executeCommand(`NM1TEST/PASSENGER`, { sessionId: sid });
      await executeCommand(`AP+9715550000`, { sessionId: sid });
  ...
  // 11. RT edge case
  { type: 'RETRIEVE_INVALID_PNR', action: async (sid, o, d) => {
